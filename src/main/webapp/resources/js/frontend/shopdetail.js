$(function() {
	...
	// $('#exchangelist').attr('href', '/o2o_war_exploded/frontend/awardlist?shopId=' +
	// shopId);
	// 获取本店铺信息以及商品类别信息列表
		...
		$
				.getJSON(
						...
						function(data) {
							if (data.success) {
								var shop = data.shop;
								$('#shop-cover-pic').attr('src', shop.image);
								$('#shop-update-time').html(
										new Date(shop.updateTime)
												.Format("yyyy-MM-dd"));
								$('#shop-name').html(shop.name);
								$('#shop-desc').html(shop.desc);
								$('#shop-addr').html(shop.address);
								$('#shop-phone').html(shop.phoneNumber);
								// 获取后台返回的该店铺的商品类别列表
			...
			if (data.success) {
				...
				data.productList.map(function(item, index) {
					html += '' + '<div class="card" data-product-id='
							...
							+ '<div class="item-subtitle">' + item.desc
							+ '</div>' + '</div>' + '</li>' + '</ul>'
