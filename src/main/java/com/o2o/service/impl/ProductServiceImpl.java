...
public class ProductServiceImpl extends BaseServiceImpl<Product,Integer> implements ProductService {
	...
	public ProductExecution modifyProduct(Product product, ImageHolder thumbnail,
		...
		if (product != null && product.getShop() != null && product.getShop().getId() != null) {
			...
			Product tempProduct = productDao.selectByPrimaryKey(product.getId());

			// 若商品缩略图不为空且原有缩略图不为空则删除原有缩略图并添加
			if (thumbnail != null) {
				if (tempProduct.getImgAddress() != null) {
					FileUtil.deleteFile(tempProduct.getImgAddress());
				}
				addThumbnail(product, thumbnail);
			}
			// 如果有新存入的商品详情图，则将原先的删除，并添加新的图片
			if (productImgHolderList != null && productImgHolderList.size() > 0) {
				deleteProductImgList(tempProduct.getProductImgList());
				addProductImgList(product, productImgHolderList);
			}
			try {
				// 更新商品信息
	...
	private void addThumbnail(Product product, ImageHolder thumbnail) {
		String dest = FileUtil.getShopImagePath(product.getShop().getId());
		String thumbnailAddr = ImageUtil.generateThumbnail(thumbnail, dest);
		product.setImgAddress(thumbnailAddr);
	}
	 ...
	 * @param
	 */
	private void deleteProductImgList(List<ProductImg> productImgList) {
//		// 根据productId获取原来的图片
//		List<ProductImg> productImgList = productImgDao.queryProductImgList(productId);
		// 干掉原来的图片
		for (ProductImg productImg : productImgList) {
			FileUtil.deleteFile(productImg.getUrl());
			// 删除数据库里原有图片的信息
			productImgDao.deleteByPrimaryKey(productImg.getId());
		}
